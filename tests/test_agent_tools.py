...
def _all_tools():
    return [T.web_search, T.fetch_page, T.validate_link, T.assign_marvel_voices,
...
def test_assign_marvel_voices_returns_distinct_personas_with_disclaimer():
    raw = T.assign_marvel_voices.invoke({"count": 3, "seed": "2026-06-02"})
    data = json.loads(raw)
    assert len(data) == 3
...
def test_assign_marvel_voices_is_day_stable():
    a = T.assign_marvel_voices.invoke({"count": 4, "seed": "2026-06-02"})
    b = T.assign_marvel_voices.invoke({"count": 4, "seed": "2026-06-02"})
    assert a == b
