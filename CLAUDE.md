...
| `content_pipeline/generate/podcast_script.py` | Dad↔son podcast script: clean two-host cold-open (Graham + date, Tom breaks in) + ALTERNATING verbatim readings (Graham/Tom; a deployed parody guest reads its own) + transitional discussion banter with **parody hand-offs** (host introduces each guest by seniority; a deployed guest may speak one gated in-character line) + Tom's minimal slang (`build_podcast_script`); plus the deterministic **<180s** `build_tldr_script` headline bulletin |
