...
Single source of truth for AI agents working in this repository, per the AGENTS.md
spec. `CLAUDE.md` is a symlink to this file.
...
homelab fleet (reached through one **LiteLLM proxy**). There is **no frontier model** in
the system — a failed call crosses to the other box, not to a hosted API. It doubles as a **LangChain deep-agent tutorial** — every file is
...
run_edition (content_pipeline/agent/editor_in_chief.py)
   ...
   │      the creator). LiteLLM brain/writer = DGX Qwen3.8-27B NVFP4, M3 cross-box fallback.
   │      MOTO items carry a hard "this is a BIKE, never a car" constraint + a
   │      car-noun check with ONE re-write (writer.car_words_in).
   ...
   │      on an INDEPENDENT local model (qwen3-coder-next, M3 :8087 — distinct from the writer; cross-box fallback).
...
| `content_pipeline/agent/tools.py` | `@tool`s: `web_search` (Serper.dev), `fetch_page`, `validate_link` — the three actually attached to subagents. `generate_cover_image` and `assign_journalist_voices` are **VESTIGIAL**: attached to nothing, and divergent from the live path (the former bypasses the style rotation and never stamps `_image_style`). Images are generated by the harness, personas by `personas.assign_personas`. |
...
| `content_pipeline/agent/rubric_review.py` | `grade_edition`: in-pipeline deepagents **RubricMiddleware** judge on an **independent** local model (`qwen3-coder-next`, M3 :8087 — Qwen3-Coder-Next 80B-A3B, distinct from the writer; cross-box fallback) → `verdict-rubric.json`; replaced the two-VM consensus. (The podcast-banter gate `grade_podcast_script` was REMOVED 2026-06-10 — too strict.) |
...
| `content_pipeline/providers/litellm.py` | `get_litellm_llm` (ChatOpenAI → LiteLLM proxy) + `run_with_fallback` (local-first → cross-box local; there is no frontier route) |
...
> **Verify routes against the proxy, not against this table.** Three sources of truth drift
> independently: these defaults, prod `/etc/craicgpt.env` on `.75`, and the fleet's
> `docs/provisioning/craicgpt-deploy.md`. In Aug 2026 prod was ahead of both. Check with
> `curl -s $LITELLM_BASE_URL/models -H "Authorization: Bearer $LITELLM_API_KEY"`.
> The proxy serves **local routes only** — there is no frontier fallback anywhere in this
> system, and `claude-sonnet-4-6` was always a dead route (HTTP 400).

| Variable | Required | Default |
|----------|----------|---------|
| `LITELLM_BASE_URL` | No | `https://llm.grazlab.thescriptingpaddy.com/v1` |
| `LITELLM_API_KEY` | No | `sk-no-key-required` |
| `BRAIN_MODEL` (research) | No | `dgx/vllm/qwen3.8-27b-nvfp4` |
| `WRITE_MODEL` (prose) | No | `dgx/vllm/qwen3.8-27b-nvfp4` |
...
| `FALLBACK_TEXT_MODEL` (local cross-box) | No | `m3/mlx/qwen3.8-27b-8bit` (MUST accept `response_format: json_schema` — the 3.6 route stalls, it does not error) |
