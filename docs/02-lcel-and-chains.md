...
llm = get_litellm_llm("dgx/vllm/qwen3.8-27b-nvfp4")   # any route on the LiteLLM proxy
...
between "Qwen3.8 on the DGX" and "Qwen3.8 on the M3" is the route string you pass.
...
## The headline act: local-first, cross-box fallback
...
fallback route — recording *which* model actually produced the output.

> Note the fallback is the **other box**, not a frontier provider: the grazlab proxy serves
> local routes only. Same weights, different engine (vLLM/NVFP4 on the DGX → MLX on the M3),
> which is what makes it a real safety net rather than a second chance at the same failure.
...
result = run_with_fallback(
    ...
    local_model="dgx/vllm/qwen3.8-27b-nvfp4",
    fallback_model="m3/mlx/qwen3.8-27b-8bit",
...
>    back to the other box only on failure, and records which one actually ran.
